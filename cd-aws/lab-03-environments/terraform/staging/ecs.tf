...
# ---------------------------------------------------------------------------
# Auto-generate task-definition.staging.json for the GitHub Actions pipeline
#
# Same approach as Lab 02: Terraform writes the file with real ARNs after
# `apply` so the pipeline never has to deal with placeholders.
# The file is written two levels up (terraform/staging/ → lab root) so it
# sits alongside the Dockerfile, exactly where the workflow expects it.
# The `image` field uses the initial bootstrap value; the pipeline replaces
# it with the commit-SHA-tagged URI at deploy time.
# ---------------------------------------------------------------------------
resource "local_file" "task_definition" {
  filename        = "${path.module}/../../task-definition.staging.json"
